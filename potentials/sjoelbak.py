...
class RectangularGridWithBarrierPotential:
    ...
    def plot_pot(self, intfs):
        ...
        for y in np.linspace(-.2*self.Lx, 1.2*self.Lx, 500):
            x_y[i] = np.array([self.potential((np.array([y, xx]), np.array([0,0]))) for xx in np.linspace(-.2*self.Ly, 1.2*self.Ly, 500)])
            i+=1
        ...
        i=0
        for y in np.linspace(-.2*self.Lx, 1.2*self.Lx, 500):
            x_y[i] = np.array([np.average(self.force((np.array([y, xx]), np.array([0,0])))) for xx in np.linspace(-.2*self.Ly, 1.2*self.Ly, 500)])
            i+=1
