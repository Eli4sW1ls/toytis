...
class PotentialTwoChannels():
    ...
    def __init__(self, desc='2D surface with 2 reaction channels'):
        ...
        self.params = {'Vmin1': 10.0, 'Vmin2': 11.0, 'Vmax': 20.0, 'L': 1.0,
                'c': 6.0 }
    ...
    def potential(self, ph):
        ...
        x = ph[0][0]  # pylint: disable=invalid-name
        y = ph[0][1]  # pylint: disable=invalid-name
    ...
    def plot_pot(self, intfs):
        ...
        for y in np.linspace(-3, 3, 500):
            x_y[i] = np.array([self.potential((np.array([xx, y]), np.array([0,0]))) for xx in np.linspace(-3, 3, 500)])
            i+=1
        c1 = ax1.pcolorfast((-3,3),(-3,3), x_y)
        for intf in intfs:
        ...
        i=0
        for y in np.linspace(-3, 3, 500):
            x_y[i] = np.array([np.average(self.force((np.array([xx, y]), np.array([0,0])))) for xx in np.linspace(-3, 3, 500)])
            i+=1
        c2 = ax2.pcolorfast((-3,3),(-3,3),  x_y)
        for intf in intfs:
